...
public class BoosterWightPlus : MonoBehaviour, IBooster
{
    private void Start()
    {
    ...
    public void Boost()
    {
        ...
        if (board.localScale.x < 8f)
        {
            board.DOScaleX(board.localScale.x + 1f, .5f).SetEase(Ease.OutBack);

            GameController.instance.board.RecoveryBoost(YandexGame.savesData.wightPlusDuration);
        }
